...
export class Search extends Component<Props, State> {
  ...
  arrowDown = () => {
    this.setState(({ hoveredIndex }: { hoveredIndex: number }) => {
      const newIndex = hoveredIndex < this.state.results.length - 1 ? hoveredIndex + 1 : 0;
  ...
  arrowUp = () => {
    this.setState(({ hoveredIndex }: { hoveredIndex: number }) => {
      const newIndex = hoveredIndex > 0 ? hoveredIndex - 1 : this.state.results.length - 1;
      return { hoveredIndex: newIndex };
    });
  };
  ...
  render() {
    const { areResultsShowing, hoveredIndex, notFound, searchText } = this.state;
    ...
    return (
      <div id='searchWrapperID' className={`searchContainer ${true ? 'pt3' : 'pt0'}`}>
        <div
          ...
          className={`innerWrapper relative ${!areResultsShowing && 'wrapperBox'}`}
        >
          <Input
